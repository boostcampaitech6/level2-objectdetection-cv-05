...
class YOLACTHead(AnchorHead):
    ...
    def get_bboxes(
        self, cls_scores, bbox_preds, coeff_preds, img_metas, cfg=None, rescale=False
    ):
...
class YOLACTProtonet(BaseModule):
            ...
            if num_pos == 0:
                ...
                loss = (
                    F.binary_cross_entropy(
                        cur_mask_pred, mask_targets, reduction="none"
                    )
    ...
    def simple_test(
        self, feats, det_bboxes, det_labels, det_coeffs, img_metas, rescale=False
    ):
        """Test function without test-time augmentation.
        ...
        if all(det_bbox.shape[0] == 0 for det_bbox in det_bboxes):
            segm_results = [
                [[] for _ in range(self.num_classes)] for _ in range(num_imgs)
            ]
        else:
