...
class OpenImagesDataset(CustomDataset):
    ...
    def get_meta_from_file(self, meta_file=""):
        ...
        metas = mmcv.load(
            meta_file, file_format="pkl", file_client_args=self.file_client_args
        )
        ...
        for i in range(len(det_results)):
            ...
            for valid_class in valid_classes:
                ...
                for index in det_cls:
                    if (
                        index in allowed_labeles
                        and index != valid_class
                        and self.get_supercategory
                    ):
                        det_results[i][index] = np.concatenate(
                            (det_results[i][index], results[valid_class])
                        )
    ...
    def evaluate(
        ...
        if self.load_image_level_labels:
            image_level_annotations = self.get_image_level_ann(
        ...
        ioa_thrs = (
            [ioa_thr] if isinstance(ioa_thr, float) or ioa_thr is None else ioa_thr
        )
        ...
        with open(image_level_ann_file, "r") as f:
            ...
            for line in reader:
                ...
                if i == 0:
                    ...
                    item_lists[img_id].append(
                        dict(image_level_label=image_level_label, confidence=confidence)
                    )
        return item_lists
