...
def test_maskformer_head_loss():
    ...
    empty_gt_losses = self.loss(
        all_cls_scores, all_mask_preds, gt_labels_list, gt_masks_list, img_metas
    )
    ...
    for key, loss in empty_gt_losses.items():
        if "cls" in key:
            ...
            assert (
                loss.item() == 0
            ), "there should be no dice loss when there are no true mask"
    ...
    self.forward_train(
        feats, img_metas, gt_bboxes, gt_labels, gt_masks, gt_semantic_seg
    )
