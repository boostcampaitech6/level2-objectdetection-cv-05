...
def test_instance_data():
    meta_info = dict(
        img_size=(256, 256), path="dadfaff", scale_factor=np.array([1.5, 1.5, 1, 1])
    )
    ...
    for key, value in long_index_instance_data.items():
        if not isinstance(value, list):
            assert (
                long_index_instance_data[key] == new_instance_data[key][long_tensor]
            ).all()
        else:
    ...
    for key, value in bool_index_instance_data.items():
        if not isinstance(value, list):
            assert (
                bool_index_instance_data[key] == new_instance_data[key][bool_tensor]
            ).all()
        else:
