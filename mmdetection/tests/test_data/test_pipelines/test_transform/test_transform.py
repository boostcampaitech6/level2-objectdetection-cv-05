...
def test_resize():
    ...
    transform = dict(
        type="Resize", img_scale=(640, 400), multiscale_mode="value", keep_ratio=False
    )
    resize_module = build_from_cfg(transform, PIPELINES)
...
def test_copypaste():
    ...
    assert (
        results["gt_bboxes"].shape[0] == dst_bboxes.shape[0] + src_bboxes.shape[0] - 1
    )
    assert (
        results["gt_labels"].shape[0] == dst_labels.shape[0] + src_labels.shape[0] - 1
    )
    assert (
