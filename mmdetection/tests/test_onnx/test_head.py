...
if digit_version(torch.__version__) <= digit_version("1.5.0"):
    pytest.skip(
        "ort backend does not support version below 1.5.0", allow_module_level=True
    )
...
def test_fcos_head_forward_single():
    ...
    fcos_model.forward_single = partial(
        fcos_model.forward_single, scale=Scale(1.0).requires_grad_(False), stride=(4,)
    )
...
def ssd_config():
    ...
    model = SSDHead(
        num_classes=4, in_channels=(4, 8, 4, 2, 2, 2), test_cfg=test_cfg, **cfg
    )
