...
try:
    ...
    raise ImportError(
        "Please run `pip install iterative-stratification` to install "
...
var = [
    (ann["image_id"], ann["category_id"], ann["bbox"]) for ann in data["annotations"]
]
...
if METHOD == 0:
    ...
    for i, (train_idx, val_idx) in enumerate(cv.split(X, y, groups), start=1):
        ...
        train_filename = os.path.join(
            output_dir, methods[METHOD], f"train_fold_{i}.json"
        )
...
# ref: https://github.com/amirassov/kaggle-global-wheat-detection/blob/master/gwd/split_folds.py
    ...
    for i, (train_idx, val_idx) in enumerate(
        cv.split(
            X,
            np.column_stack((ms["categories"], ms["median_area"], ms["num_of_bboxes"])),
        ),
        start=1,
    ):
        train_data_img_ids = set(
